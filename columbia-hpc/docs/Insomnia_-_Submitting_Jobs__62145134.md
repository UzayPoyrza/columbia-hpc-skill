# Insomnia - Submitting Jobs

Source: https://columbiauniversity.atlassian.net/wiki/spaces/rcs/pages/62145134/Insomnia+-+Submitting+Jobs

---

- [Processes and Jobs (sbatch, srun, and more)](#Insomnia-SubmittingJobs-ProcessesandJobs(sbatch,srun,andmore))
- [Restrictions on Login Node Usage](#Insomnia-SubmittingJobs-RestrictionsonLoginNodeUsage)
- [Core-Based Jobs](#Insomnia-SubmittingJobs-Core-BasedJobs)
- [Node-Based Jobs](#Insomnia-SubmittingJobs-Node-BasedJobs)
- [Interactive Jobs](#Insomnia-SubmittingJobs-InteractiveJobs)
- [Basic Job Directives](#Insomnia-SubmittingJobs-BasicJobDirectives)
- [Walltime](#Insomnia-SubmittingJobs-Walltime)
- [Memory Requests](#Insomnia-SubmittingJobs-MemoryRequests)
- [Job Arrays](#Insomnia-SubmittingJobs-JobArrays)
- [Job scheduling basics](#Insomnia-SubmittingJobs-Jobschedulingbasics)
- [Job limits](#Insomnia-SubmittingJobs-Joblimits)
- [squeue Status and Reason Codes](#Insomnia-SubmittingJobs-squeueStatusandReasonCodes)
- [Slurm Quick Start Guide](#Insomnia-SubmittingJobs-SlurmQuickStartGuide)
...
[https://columbiauniversity.atlassian.net/wiki/display/rcs/Insomnia+-+Submitting+Jobs#Insomnia-SubmittingJobs-InteractiveJobs](https://columbiauniversity.atlassian.net/wiki/display/rcs/Insomnia+-+Submitting+Jobs)

Jobs can request compute resources on a per-core basis or a per-node basis.

## Core-Based Jobs

Jobs wishing to use less than a full node should specify the number of cores required. If you're not sure how many cores to request then 1 is most likely to be the correct number to use. The maximum number of cores available on a node is 80.
...
```
#SBATCH --mem=510G    # Standard nodes have approximately 510G of total memory available
```

To explicitly request a "standard node" and 400 GB RAM, you may specify the mem400 feature.

```
#SBATCH -C mem400
```

If your job requires more than the standard 512 GB then you may optionally add a constraint to request one of the cluster's high memory nodes, each of which has 1024 GB (i.e. 1TB) of memory. The feature to request is "mem1024".
...
```
srun --pty -t 0-01:00 -C mem1024 -A <ACCOUNT> /bin/bash
```
...
| --cpus-per-task=<cpus> | -c <cpus> | CPU cores per task. | #SBATCH -c 1 | Nodes have a maximum of 80 cores. |
...
At this writing (December 2025), Insomnia has 41 Standard Nodes with 512 GB of memory (approx 510 GB usable).

Insomnia's19 high memory nodes have 1TB of memory each. They are otherwise identical to the standard nodes.  
For example:
...
Minimum memory required per allocated CPU. If you request all 80 cores (one node) you will get about 400gb of memory on both standard node and on high-memory node.   
...
Note: There is a **limit of 500 max jobs running per user**. Any jobs that exceed this limit will remain in queue until (some of) the user's other running jobs complete.
